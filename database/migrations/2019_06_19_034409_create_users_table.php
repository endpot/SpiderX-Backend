...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('passkey', '32')->unique()->comment('Tracker 通信密钥');
            $table->tinyInteger('state')->default(0)->comment('账号状态');

            $table->timestamp('last_login_date')->nullable()->comment('最近登录时间');
            $table->timestamp('last_access_date')->nullable()->comment('最近访问时间');
            $table->ipAddress('last_access_ip')->nullable()->comment('最近访问IP');

            $table->unsignedTinyInteger('class')->default(1)->comment('用户等级');
            $table->unsignedTinyInteger('max_class_once')->default(1)->comment('曾经最高等级');

            $table->string('title')->default('')->comment('头衔');
            $table->string('avatar')->default('')->comment('头像');
            $table->text('info')->nullable()->comment('个人说明');
            ...
            $table->timestamp('vip_added_at')->nullable()->comment('VIP 添加日期');
            $table->timestamp('vip_until')->nullable()->comment('VIP 有效期');
            ...
            $table->timestamp('warned_at')->nullable()->comment('警告日期');
            $table->timestamp('warned_by')->nullable()->comment('警告人');
            $table->timestamp('warned_until')->nullable()->comment('警告截止日期');
